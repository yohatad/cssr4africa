...
from person_detection.msg import personDetection
...
class PersonDetectionNode:
    def __init__(self, config):
        ...
        self.pub_people = rospy.Publisher("/personDetection/data", personDetection, queue_size=10)
    ...
    def parse_config():
        ...
        try:
            ...
            config_path = os.path.join(package_path, 'config', 'peerson_detection_configuration.ini')
...
class YOLOv8ROS():
    ...
    def detect_object(self, image):
        ...
        outputs = self.sixdrepnet_session.run(self.output_names, {self.input_names[0]: processed_frame})
    ...
    #     """"
    #     Process the input frame for person detection.
    #     Args:
    #         cv_image: Input frame in OpenCV format.
    #     """
    ...
    def image_callback(self, msg):
        ...
        cv2.imshow("YOLOv8 ROS", frame_with_detections)
        cv2.waitKey(1)
        ...
        while not rospy.is_shutdown():
            if self.latest_frame is not None:
                if self.verbose_mode:
                    cv2.imshow("Person Detection", self.latest_frame)
