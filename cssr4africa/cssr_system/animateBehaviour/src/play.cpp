...
void moveToPositionBiological(ControlClientPtr& client, 
    ...
    double totalGestureDuration = cumulativeTime + 1.0; // Add a 1-second buffer for timeout
...
void generateListOfRandomPositions(std::vector<std::vector<double>>& positions, 
    ...
    for (int i = 0; i < numberOfPositions; ++i) {
        ...
        if (verboseMode) {
            ROS_INFO("Calculating target position %d", i + 1);
        }
        ...
        // Optional: Add a small delay to allow time to progress if necessary (could be removed)
...
void computeTrajectoryCubicSpline(const std::vector<std::vector<double>>& randomPositions,
    ...
    // Clear the previous trajectory data
    positions_t.clear();
    velocities_t.clear();
    accelerations_t.clear();
    durations.clear();

    // Ensure positions_t and others are resized appropriately
    positions_t.resize(number_of_positions, std::vector<double>(number_of_joints, 0.0));
    ...
    std::vector<Eigen::VectorXd> spline_coefficients_a(number_of_joints); // a(t)
    std::vector<Eigen::VectorXd> spline_coefficients_b(number_of_joints); // b(t)
    std::vector<Eigen::VectorXd> spline_coefficients_c(number_of_joints); // c(t)
    std::vector<Eigen::VectorXd> spline_coefficients_d(number_of_joints); // d(t)

    for (size_t joint = 0; joint < number_of_joints; ++joint) {
        // Extract the position data for the specific joint
        ...
        // Calculate the remaining coefficients a, b, and d
        Eigen::VectorXd a = positions.segment(0, number_of_positions - 1);
    ...
    // Now that we have the spline coefficients, we can generate the trajectory for each joint
    for (size_t i = 0; i < number_of_positions; ++i) {
        ...
        for (size_t joint = 0; joint < number_of_joints; ++joint) {
            ...
            // Compute positions, velocities, and accelerations for each time step
            positions_t[i][joint] = spline_coefficients_a[joint](i) + 
                                    spline_coefficients_b[joint](i) * dt + 
                                    spline_coefficients_c[joint](i) * dt * dt + 
                                    spline_coefficients_d[joint](i) * dt * dt * dt;
...
void lArm(ros::NodeHandle& nh, std::string leftArmTopic, bool resetPosition) {
    ...
    double baseTimeFactor = configParams.find("baseTimeFactor") != configParams.end() 
                            ? std::stod(configParams["baseTimeFactor"]) 
                            : 1.0;
    ...
    computeDurationsBasedOnDistance(randomPositions, duration_t, baseTimeFactor);

    // Compute the trajectory using cubic spline interpolation
    computeTrajectoryCubicSpline(randomPositions, currentPosition, duration_t.back(), positions_t, velocities_t, accelerations_t, duration_t);
