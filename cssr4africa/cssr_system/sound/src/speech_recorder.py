...
class SpeechRecorder:
    def __init__(self, vad, buffer_duration=0.5, silence_duration=0.5, chunk_size=0.1, vad_window=0.2, sample_rate=16000):
    ...
    def vad_thread_func(self):
        while True:
            if len(self.audio_buffer) >= int(self.vad_window * self.sample_rate):
                with self.buffer_lock:
                ...
                if self.vad(vad_chunk):
                    ...
                    # If not recording, keep the buffer at max buffer_duration
                    self.recording_data = np.concatenate((self.recording_data, chunk))
...
if __name__ == "__main__":
    ...
    Vad = SileroVad("/home/yoha/workspace/pepper_rob_ws/src/cssr4africa/cssr_system/sound/models/silero_vad.jit", 0.8)
