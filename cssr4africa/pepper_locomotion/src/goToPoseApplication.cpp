/*******************************************************************************************************************
*
*  Implementation of the divide-and-conquer go-to-position algorithm
*              and the MIMO go-to-position algorithm (goto2) 
*              on the Pepper Social robot with algorithms for testing the odometry
*
*  P.S. The package directory was hardcoded in the application file, adjust based on the directory of your workspace
*
...
*  a key string  (either "setpose", "goto1", "goto2", "testlinearodom" or "testangularodom") followed by three floating point numbers.
*  
...
*
*  In the case of the "testlinearodom" command, the three numbers define the goal pose of the robot. Although, only the first value (x) is of importance here.
*  The command moves the robot forward in the x-direction to a particular x-value in meters
*
*  In the case of the "testangularodom" command, the three numbers define the goal pose of the robot. Although, only the third value (theta) is of importance here.
*  The command rotates the robot through some angle about the z-direction to a particular angle in radians
*
...
*   parameters.txt
*   setpose             0     0     0
*   testlinearodom     1.2   0     0
*   testangularodom     0     0     3.142
*   goto1               0     1.2   0
*   goto2               0     0     0
*
*
...
#include <pepper_locomotion/goToPose.h>
...
main(int argc, char **argv)
{
   ...
   FILE *fp_in;
   std::string packagedir = "/home/yoha/workspace/pepper_rob_ws/src/cssr4africa/pepper_locomotion";          // Package directory: Adjust based on your workspace
   ...
   if (debug)
      printf("Locomotion parameter filename %s\n", locomotion_parameter_filename);
   ...
   if (debug)
      printf("Locomotion parameter file is  %s\n", path_and_input_filename);
   ...
   float max_linear_velocity = 0.55;
   float max_angular_velocity = 0.55;
   ...
   while (end_of_file != EOF)
   {
      ...
      else if (strcmp(command, "testlinearodom") == 0)
      {

         /* Test the odometry by driving the robot forward */

         testLinearOdometry(x, locomotionParameterData, pub, rate);
      }
      else if (strcmp(command, "testangularodom") == 0)
      {
         /* Test the odometry by rotating the robot on a spot */

         testAngularOdometry(theta, locomotionParameterData, pub, rate);
      }
