...
void goToPoseDQ(float x, float y, float theta, locomotionParameterDataType locomotionParameterData, ros::Publisher pub, ros::Rate rate) {
   ...
   //Stop the robot at its destination
   msg.angular.z = 0;
   msg.linear.x = 0;
   pub.publish(msg);              // Publish the message
   rate.sleep(); // Wait until it's time for another iteration
...
void goToPoseMIMO1(float x, float y, float theta, locomotionParameterDataType locomotionParameterData, ros::Publisher pub, ros::Rate rate) {
   ...
   do {
	 ...
	 for (int i=1; i < number_of_ramp_up_steps; i+=2) {
            msg.linear.x = (float) linear_velocity * ((float) i / (float) number_of_ramp_up_steps);
	    msg.angular.z = 0;
...
Test the accuracy of the odometry in linear forward movement of the robot.
Uses same input file, but only uses the x-value of the input file

***********************************************************************************************************************/

void testLinearOdometry(float x, locomotionParameterDataType locomotionParameterData, ros::Publisher pub, ros::Rate rate)
{
   ...
   do
   {
      ...
      position_error = fabs(goal_x - current_x);
...
Test the accuracy of the odometry in angular rotation movement of the robot.
Uses same input file, but only uses the theta value of the input file

***********************************************************************************************************************/

void testAngularOdometry(float theta, locomotionParameterDataType locomotionParameterData, ros::Publisher pub, ros::Rate rate)
{
